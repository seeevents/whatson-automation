...
- SI ET SEULEMENT SI cette seconde recherche ne renvoie toujours aucun resultat confirme -> VERIFICATION SUPPLEMENTAIRE avant de creer : parmi TOUS les candidats trouves par les recherches ci-dessus (meme non confirmes par urlEvent - ceci couvre le cas d'un event cree manuellement par l'equipe, sans urlEvent renseigne), y a-t-il un candidat sur la MEME DATE dont le titre semble parler du MEME sujet/evenement reel (meme artiste, meme type de soiree, meme occasion) meme si le titre exact differe ? Si oui : cree quand meme le nouvel evenement normalement (ne fusionne pas automatiquement, trop risque de melanger deux evenements reellement differents), MAIS ajoute dans le champ "message" de ta reponse finale une alerte explicite du type "DOUBLON POTENTIEL: event existant similaire trouve (id XXXXX, titre 'YYYY') a la meme date - verifier manuellement et fusionner/supprimer si besoin." pour que l'equipe le reperer facilement en revision. Si aucun candidat de ce type -> cree normalement sans alerte.
