...
- SI des resultats sont renvoyes :
  1. Limite-toi aux 3 candidats les plus pertinents/recents (pas plus, meme si la liste en contient davantage). Pour chacun, verifie la presence du champ urlEvent. Si urlEvent n'est pas present dans la liste, appelle cms_get_event(id) pour l'obtenir - UNE SEULE FOIS par candidat, jamais deux fois pour le meme id.
...
- SI ET SEULEMENT SI cette seconde recherche ne renvoie toujours aucun resultat confirme -> verifie (a partir des donnees DEJA obtenues ci-dessus, sans nouvel appel) si un candidat sur la MEME DATE semble parler du MEME sujet/evenement reel malgre un titre different (ex: event cree manuellement sans urlEvent). Si oui : cree quand meme normalement, mais ajoute dans "message" : "DOUBLON POTENTIEL: event existant similaire trouve (id XXXXX, titre 'YYYY') a la meme date - verifier manuellement." Sinon, cree normalement sans alerte.

Etape 2 (Mise a jour si l'evenement existe) :
- Choisis l'evenement LE PLUS ANCIEN parmi les resultats deja obtenus a l'Etape 1 (ne refais pas de recherche).
- Fais UN SEUL appel cms_get_event sur cet ID pour recuperer en une fois : categories actuelles, date de debut actuelle (a garder en tete pour le champ "date_changed"), et le contenu du paragraphe de texte existant (ou cms_list_event_paragraphs seulement si cms_get_event ne renvoie pas les paragraphes). Ne fais pas d'appels separes pour chaque info si tu peux les obtenir ensemble.
- Decide en UNE PASSE, a partir de ce que tu viens de lire :
  - CAS FUSION (l'evenement existant semble deja parler du meme festival/soiree mais mentionne un artiste/exposant DIFFERENT non present dans le nouveau texte source) : ajoute le nouvel element a la suite du texte existant (garde tout, separe par un saut de ligne). Si l'element est deja mentionne, ne duplique pas.
  - CAS REMPLACEMENT NORMAL (par defaut, dans tous les autres cas) : remplace le texte par le nouveau.
- Execute cms_update_event (titre, dates, SEO, categories = celles deja notees moins la date + la nouvelle categorie de date, Image URL sauf cas fusion ou l'image existante reste pertinente) ET cms_update_event_paragraph (texte fusionne ou remplace) - ne touche JAMAIS a la categorie 'Top Events' (10683231).
...
- SEO META DESCRIPTION (OBLIGATOIRE, respecte la longueur cible) : Genere la description en suivant exactement cette consigne : "Write a SEO meta description for a [description de l'evenement] event in [description de la venue] in Bali in 120 characters in english and using SEO longtail". La cible est ~120 caracteres, PAS un plafond a ne pas depasser - ne t'arrete pas prematurement a 60-80 caracteres, utilise vraiment l'espace disponible pour une description riche en mots-cles longue traine. Puis ajoute " #seeeventsbali #ifyouseeyouknow" a la fin (hors des 120 caracteres cibles).
